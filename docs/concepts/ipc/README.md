...
The preload script [src/preload.ts](../../../src/preload.ts) uses Electron's `contextBridge` to expose a restricted API window (`window.chrono`) to the renderer. 
...
All communication across the IPC boundary is governed by [contract.ts](../../../src/shared/contract.ts).
