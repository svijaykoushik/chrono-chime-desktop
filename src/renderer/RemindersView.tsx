...
export function RemindersView({ tz }: { tz: string }) {
  ...
  const bulkDelete = async () => {
    await window.chrono.reminders.delete(ids);
    setSelection(new Set());
    ...
    <Box sx={{ pb: 10 }}>
      {selectionMode ? (
        <Toolbar sx={{ bgcolor: 'primary.main', color: 'primary.contrastText', borderRadius: 2, mb: 2 }}>
          ...
          <IconButton color="inherit" onClick={() => setConfirmDelete(true)} aria-label="delete selected"><DeleteIcon /></IconButton>
          ...
          {reminders.map((r) => (
            ...
            >
              <CardContent sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                ...
                {!selectionMode && <Switch checked={r.enabled} onChange={() => toggleEnabled(r)} />}
              </CardContent>
            </Card>
          ))}
        </Stack>
      ...
      <Fab color="primary" aria-label="add" sx={{ position: 'fixed', bottom: 24, right: 24 }} onClick={() => setDialogOpen(true)}>
        <AddIcon />
      </Fab>

      <ReminderDialog
        open={dialogOpen} tz={tz}
        onClose={() => setDialogOpen(false)}
        onSave={async (input) => { await window.chrono.reminders.create(input); void refresh(); }}
      />

      <Dialog open={confirmDelete} onClose={() => setConfirmDelete(false)}>
        <DialogTitle>Delete {selection.size} reminder(s)?</DialogTitle>
        <DialogContent><DialogContentText>This cannot be undone.</DialogContentText></DialogContent>
        <DialogActions>
          <Button onClick={() => setConfirmDelete(false)}>Cancel</Button>
          <Button color="error" variant="contained" onClick={bulkDelete}>Delete</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
